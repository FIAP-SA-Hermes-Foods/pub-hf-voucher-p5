...
import (
	...
	"pub-hf-voucher-p5/internal/core/domain/entity"
...
func (h *handler) getVoucherByID(rw http.ResponseWriter, req *http.Request) {
	...
	v:= h.app.GetVoucherByID(msgID, id)
...
func (h *handler) saveVoucher(rw http.ResponseWriter, req *http.Request) {
	...
	_, err := strconv.ParseInt(reqVoucher.Percentage, 10, 64)
	if err != nil {
		rw.WriteHeader(http.StatusInternalServerError)
	...
	v:= h.app.SaveVoucher(msgID, reqVoucher)
...
func (h *handler) updateVoucherByID(rw http.ResponseWriter, req *http.Request) {
	...
	v:= h.app.UpdateVoucherByID(msgID, id, reqVoucher)
